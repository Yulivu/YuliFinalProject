"""
改进版优化ResNet思想的地震断层运动方式识别及板块边界预测模型
- 基于v3c1的改进
- 使用深度可分离卷积减少参数量
- 添加SE模块和注意力机制提高特征提取能力
- 使用增强版Focal Loss和样本权重解决类别不平衡问题
- 优化早停策略和学习率调度
"""
...
OUTPUT_DIR = r"C:\Users\debuf\Desktop\YuliFinalProject\result\v3c1_optimized"
...
def prepare_windowed_data(eq_data, boundary_data, masks, window_size=7):
    ...
    # 由于类别严重不平衡，增强权重
    pos_weight = max(pos_weight * 1.5, 10.0)  # 确保正样本至少有10倍的权重
    print(f"Enhanced positive class weight: {pos_weight:.2f}")
...
def squeeze_excite_block(input_tensor, ratio=8):  # 降低ratio以增强SE效果
    """挤压激励(SE)模块"""
...
def spatial_attention(x, reduction_ratio=4):  # 降低ratio以增强注意力效果
    """空间注意力机制"""
    # 使用1x1卷积进行通道降维
    channel_dim = x.shape[-1]
    reduced = layers.Conv2D(channel_dim // reduction_ratio, (1, 1), padding='same')(x)
    reduced = layers.Activation('relu')(reduced)

    # 再次用1x1卷积进行注意力权重生成
    attention = layers.Conv2D(channel_dim, (1, 1), padding='same')(reduced)
...
def se_residual_block(x, filters, kernel_size=3, l2_reg=1e-4):
    """带SE模块的残差块"""
    reg = tf.keras.regularizers.l2(l2_reg)

    # 残差路径
    residual = layers.SeparableConv2D(filters, kernel_size, padding='same',
                                     depthwise_regularizer=reg, pointwise_regularizer=reg)(x)
    residual = layers.BatchNormalization()(residual)
    residual = layers.Activation('relu')(residual)

    residual = layers.SeparableConv2D(filters, kernel_size, padding='same',
                                     depthwise_regularizer=reg, pointwise_regularizer=reg)(residual)
    residual = layers.BatchNormalization()(residual)
    ...
    if x.shape[-1] != filters:
        shortcut = layers.Conv2D(filters, (1, 1), padding='same', kernel_regularizer=reg)(x)
...
def residual_block_with_attention(x, filters, kernel_size=3, l2_reg=1e-4):
    """带注意力机制的残差块"""
    reg = tf.keras.regularizers.l2(l2_reg)
    ...
    # 添加空间注意力
    residual = spatial_attention(residual)
...
def focal_loss(alpha=0.75, gamma=2.5):  # 增强alpha和gamma参数
    """增强版Focal Loss，更好地处理类别不平衡问题"""
    def loss_function(y_true, y_pred):
        # 避免数值不稳定
        y_pred = tf.clip_by_value(y_pred, 1e-7, 1 - 1e-7)

        # 对于正样本(y=1)和负样本(y=0)计算不同的损失
        ...
        # 计算最终损失
        loss = -tf.reduce_sum(pt_1 * tf.math.pow(1. - y_pred, gamma) * tf.math.log(y_pred)) - \
               tf.reduce_sum(pt_0 * tf.math.pow(y_pred, gamma) * tf.math.log(1. - y_pred))
...
# 自定义F1分数计算 - 用于替代TF内置的F1Score，解决维度问题
...
def build_optimized_resnet_model(input_shape, dropout_rate=0.4):  # 增加dropout防止过拟合
    """构建优化版基于ResNet思想的CNN模型"""
    print("构建优化版ResNet模型...")

    # 设置L2正则化系数
    l2_reg = 2e-4  # 增加L2正则化强度
    reg = tf.keras.regularizers.l2(l2_reg)
    ...
    # 初始卷积层
    x = layers.Conv2D(64, (3, 3), padding='same', kernel_regularizer=reg)(inputs)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)

    # 第一组残差块 - 使用SE模块
    x = se_residual_block(x, 64, l2_reg=l2_reg)
    x = se_residual_block(x, 64, l2_reg=l2_reg)

    # 第二组残差块 - 使用SE模块
    x = se_residual_block(x, 128, l2_reg=l2_reg)
    x = se_residual_block(x, 128, l2_reg=l2_reg)

    # 第三组残差块 - 使用空间注意力
    x = residual_block_with_attention(x, 256, l2_reg=l2_reg)
    x = residual_block_with_attention(x, 256, l2_reg=l2_reg)
    ...
    # 使用Focal Loss而不是普通的二元交叉熵
    focal_loss_fn = focal_loss(alpha=0.75, gamma=2.5)

    # 编译模型 - 使用自定义的f1_score替代TF内置的F1Score
    model.compile(
...
def main():
    """主函数：完整的优化模型训练和评估工作流程"""
    print("Starting optimized ResNet-style plate boundary prediction process...")
    ...
    # 步骤 4: 准备窗口化数据
    window_size = 7  # 7x7 窗口
    X_train, X_val, y_train, y_val, pos_weight = prepare_windowed_data(
        eq_data, boundary_data, masks, window_size=window_size
    )

    # 步骤 5: 训练和评估优化模型
    model, history, model_id, run_output_dir = train_and_evaluate_optimized(
        X_train, X_val, y_train, y_val, pos_weight,
        epochs=30,  # 减少最大迭代次数
        batch_size=64,
        patience=8   # 略微降低patience值，更快触发早停
    )
    ...
    test_metrics = visualize_test_predictions(
        ...
        threshold=0.2,  # 使用较低的阈值，提高召回率
        output_path_prefix=output_path_prefix
    )

    print("Optimized ResNet-style plate boundary prediction process completed!")
...
if __name__ == "__main__":
    ...
    plt.title(f'Test Region Evaluation - Threshold={threshold} - Precision: {precision:.3f}, Recall: {recall:.3f}, F1: {f1:.3f}',
              fontsize=14)
    ...
    for thresh in thresholds:
        test_pred_at_thresh = (prediction_map[test_indices] >= thresh).astype(np.int32)

        tp = np.sum((test_true == 1) & (test_pred_at_thresh == 1))
        fp = np.sum((test_true == 0) & (test_pred_at_thresh == 1))
        fn = np.sum((test_true == 1) & (test_pred_at_thresh == 0))
    ...
    plt.plot(thresholds_df['threshold'], thresholds_df['precision'], 'b-', marker='o', label='Precision')
    plt.plot(thresholds_df['threshold'], thresholds_df['recall'], 'g-', marker='s', label='Recall')
    plt.plot(thresholds_df['threshold'], thresholds_df['f1'], 'r-', marker='^', label='F1')
    ...
    plt.tight_layout()
    plt.savefig(f"{output_path_prefix}_threshold_analysis.png", dpi=300)
    plt.close()

    # 保存阈值分析结果
    thresholds_df.to_csv(f"{output_path_prefix}_threshold_analysis.csv", index=False)
    ...
    return metrics
    print("Starting training of optimized ResNet model...")
    print(f"Training set shape: {X_train.shape}, Validation set shape: {X_val.shape}")

    # 生成时间戳作为模型ID
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_id = f"optimized_resnet_plate_boundary_model_{timestamp}"
    ...
    # 构建优化的ResNet模型
    model = build_optimized_resnet_model(X_train.shape[1:])
    model.summary()

    # 设置回调函数，使用改进的早停策略
    callbacks_list = [
        callbacks.EarlyStopping(
            monitor='val_f1_score',  # 监控F1分数
            mode='max',              # 我们希望最大化F1分数
            patience=patience,
            min_delta=0.001,         # 只有超过0.001的改善才会被视为有效
            restore_best_weights=True,
            verbose=1
        ),
        callbacks.ModelCheckpoint(
            filepath=model_path,
            monitor='val_f1_score',  # 保存F1分数最高的模型
            mode='max',
            save_best_only=True
        ),
    ...
    # 设置类别权重 - 与pos_weight结合使用，进一步强调少数类
    ...
    # 评估模型
    val_loss, val_acc, val_precision, val_recall, val_auc, val_f1 = model.evaluate(X_val, y_val, verbose=0)
    ...
    plt.tight_layout()
    f1_auc_plot_path = os.path.join(run_output_dir, 'f1_auc_history.png')
