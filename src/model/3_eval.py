...
def evaluate_model(model_path, output_tag):
    ...
    with rasterio.open(tif_path) as src:
        boundary_type_code = src.read(37)  # 波段37为boundary_type_code
    # 只统计test区域
    test_mask = masks['test']
    convergent_mask = np.isin(boundary_type_code, [0, 1]) & test_mask
    divergent_mask = np.isin(boundary_type_code, [2, 3]) & test_mask
    transform_mask = np.isin(boundary_type_code, [4, 5]) & test_mask
    # 绘制概率分布直方图
    plt.figure(figsize=(8, 5))
    plt.hist(prediction_map[convergent_mask], bins=50, alpha=0.5, label='Convergent')
    plt.hist(prediction_map[divergent_mask], bins=50, alpha=0.5, label='Divergent')
    plt.hist(prediction_map[transform_mask], bins=50, alpha=0.5, label='Transform')
    plt.xlabel('Prediction Probability')
    plt.ylabel('Count')
    plt.title('Probability Distribution by Plate Boundary Type (Test Region)')
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(run_output_dir, 'boundary_type_probability_hist.png'), dpi=300)
    plt.close()
    # 统计三大类+inferred的像元数
    inferred_mask = (~(convergent_mask | divergent_mask | transform_mask)) & test_mask
    sizes = [np.sum(convergent_mask), np.sum(divergent_mask), np.sum(transform_mask), np.sum(inferred_mask)]
    labels = ['Convergent', 'Divergent', 'Transform', 'Inferred']
    colors = ['#4F81BD','#C0504D','#9BBB59','#8064A2']
    # 绘制饼图并保存到run_output_dir
    plt.figure(figsize=(6,6))
    plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=colors)
    plt.title('Boundary Type Distribution (Test Region)')
    plt.tight_layout()
    plt.savefig(os.path.join(run_output_dir, 'boundary_type_pie.png'), dpi=300)
    plt.close()
    # 1. 全数据集三大类+inferred饼图
    csv_path = r"C:\Users\debuf\Desktop\YuliFinalProject\data\v2_processing\v2better\earthquake_features_enhanced.csv"
    df = pd.read_csv(csv_path)
    counts = df['boundary_category'].value_counts(dropna=False)
    counts.index = counts.index.fillna('inferred')
    labels = ['convergent', 'divergent', 'transform', 'inferred']
    sizes = [counts.get(l, 0) for l in labels]
    colors = ['#4F81BD','#C0504D','#9BBB59','#8064A2']
    plt.figure(figsize=(6,6))
    plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=colors)
    plt.title('Boundary Type Distribution (All Data)')
    plt.tight_layout()
    plt.savefig(os.path.join(run_output_dir, 'boundary_type_pie_all_data.png'), dpi=300)
    plt.close()

    # 2. train/val/test区域三大类+inferred饼图
    for mask, region_name in zip([masks['train'], masks['val'], masks['test']], ['train', 'val', 'test']):
        convergent_mask = np.isin(boundary_type_code, [0, 1]) & mask
        divergent_mask = np.isin(boundary_type_code, [2, 3]) & mask
        transform_mask = np.isin(boundary_type_code, [4, 5]) & mask
        inferred_mask = (~(convergent_mask | divergent_mask | transform_mask)) & mask
        sizes = [np.sum(convergent_mask), np.sum(divergent_mask), np.sum(transform_mask), np.sum(inferred_mask)]
        plt.figure(figsize=(6,6))
        plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=colors)
        plt.title(f'Boundary Type Distribution ({region_name.capitalize()} Region)')
        plt.tight_layout()
        plt.savefig(os.path.join(run_output_dir, f'boundary_type_pie_{region_name}.png'), dpi=300)
        plt.close()

    # 3. test区域三大类+inferred的precision/recall/F1统计
    print("\n[Per-Type Metrics in Test Region]")
    for mask, name in zip([convergent_mask, divergent_mask, transform_mask, inferred_mask], labels):
        y_true = test_true_mask[mask]
        y_pred = test_pred_mask[mask]
        if y_true.size > 0:
            p = precision_score(y_true, y_pred)
            r = recall_score(y_true, y_pred)
            f = f1_score(y_true, y_pred)
            print(f"{name}: Precision={p:.3f}, Recall={r:.3f}, F1={f:.3f}, N={y_true.size}")
        else:
            print(f"{name}: 无样本")

    # 4. test区域三大类+inferred的概率分布箱线图
    data = []
    for mask, name in zip([convergent_mask, divergent_mask, transform_mask, inferred_mask], labels):
        data.extend([(name, p) for p in prediction_map[mask]])
    df_box = pd.DataFrame(data, columns=['Type', 'Probability'])
    plt.figure(figsize=(8,6))
    sns.boxplot(x='Type', y='Probability', data=df_box)
    plt.title('Prediction Probability by Boundary Type (Test Region)')
    plt.tight_layout()
    plt.savefig(os.path.join(run_output_dir, 'boundary_type_boxplot.png'), dpi=300)
    plt.close()

    # ========== A. 每类概率均值、方差、极端值比例 可视化 ==========
    # Violin plot
    plt.figure(figsize=(8,6))
    sns.violinplot(x='Type', y='Probability', data=df_box, inner='quartile', palette=colors)
    plt.title('Prediction Probability by Boundary Type (Test Region) - Violin')
    plt.tight_layout()
    plt.savefig(os.path.join(run_output_dir, 'boundary_type_violinplot.png'), dpi=300)
    plt.close()
    ...
    for mask, name in zip([convergent_mask, divergent_mask, transform_mask, inferred_mask], labels):
        ...
        if y_true.size > 0:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(run_output_dir, f'confusion_matrix_{name.lower()}.png'), dpi=300)
            plt.close()

    # ========== C. error heatmap最终版可视化（灰色背景+等高colorbar+一致底色） ==========
    from scipy.ndimage import distance_transform_edt
    test_true_mask = (boundary_data[0] > 0) & test_mask
    test_pred_mask = (binary_prediction > 0) & test_mask
    dist_true = distance_transform_edt(~test_true_mask)
    dist_pred = distance_transform_edt(~test_pred_mask)
    error_map = np.zeros_like(test_true_mask, dtype=np.float32)
    error_map[test_pred_mask] = dist_true[test_pred_mask]
    error_map[test_true_mask] = dist_pred[test_true_mask]
    vmax = np.percentile(error_map[error_map > 0], 95) if np.any(error_map > 0) else 1
    # 设置plasma色表的bad为灰色
    plasma = plt.get_cmap('plasma').copy()
    plasma.set_bad(color='#b3b3b3')  # 0.7灰色
    fig, ax = plt.subplots(figsize=(12, 8))
    # test区域内error heatmap，非test区域为灰色
    masked_error = np.ma.masked_where(~test_mask, error_map)
    im = ax.imshow(masked_error, cmap=plasma, alpha=1.0, vmin=0, vmax=vmax)
    # 真实边界像元上用不同颜色scatter覆盖
    type_colors = {
        'convergent': [79/255, 129/255, 189/255],
        'divergent': [192/255, 80/255, 77/255],
        'transform': [155/255, 187/255, 89/255],
        'inferred': [128/255, 100/255, 162/255]
    }
    code_map = [ [0,1], [2,3], [4,5], [-1] ]
    for code, color, name in zip(code_map, type_colors.values(), type_colors.keys()):
        mask = np.isin(boundary_type_code, code) & (boundary_data[0] > 0) & test_mask
        yx = np.argwhere(mask)
        if yx.size > 0:
            ax.scatter(yx[:,1], yx[:,0], s=6, c=[color], label=name, alpha=1, marker='s')
    # colorbar等高
    divider = make_axes_locatable(ax)
    ...
    # 图例
    handles = [mpatches.Patch(color=type_colors[name], label=name) for name in type_colors]
    ax.legend(handles=handles, loc='upper right', fontsize=10)
    ax.set_title('Error Heatmap with Colored Boundary Types (Test Region)')
    ax.axis('off')
    plt.tight_layout()
    plt.savefig(os.path.join(run_output_dir, 'error_heatmap_colored_boundary_types_final.png'), dpi=300)
    plt.close()
