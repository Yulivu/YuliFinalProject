...
MODEL_PATH = r"C:\Users\debuf\Desktop\YuliFinalProject\src\model\models\unet_32_20250506_013740.h5"
...
def main():
    ...
    run_output_dir = os.path.join(NEW_EVAL_ROOT, f"run_{timestamp}")
    ...
    with open(os.path.join(run_output_dir, 'config.txt'), 'w') as f:
        f.write(str(config))
    print(f"Evaluation finished! F1: {f1:.4f}, Fuzzy F1: {fuzzy_f1:.4f}, Hausdorff: {hausdorff:.2f}, results saved in: {run_output_dir}")
